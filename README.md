...
4. **Calculates Fastest Multi-Agency Emergency Routes** using Open Source Routing Machine (OSRM) for both fire brigades and emergency medical ambulances.
5. **Autonomously Dispatches Alerts & Medical Warnings** via SMS/Email to fire stations, nearest hospitals/trauma centres, forest officials, and disaster response teams without manual intervention.
...
flowchart TD
    ...
    subgraph Rapid_Response ["4. Autonomous Response, Routing & Medical Alerts"]
        D1[OSRM Multi-Routing<br/>Fire Tender & Ambulance Paths]
        D2[Multi-Agency Alert Gateway<br/>Fire Stations, Hospitals & NDRF]
    end
...
| 🏥 **Medical Help & Hospital Readiness Alert** | Automatically pinpoints closest trauma centers and civil hospitals; broadcasts burn unit readiness warnings, estimated smoke spread, and ambulance navigation paths. |
...
└── AgniKavach/
    ...
    │   ├── firms_client.py       # NASA FIRMS API fetcher (Dual Mode)
    │   ├── osm_loader.py         # OpenStreetMap infrastructure & hospital parser
    │   └── spatial_crossref.py   # Spatial cross-referencing & LULC engine
    │
    ├── database/                 # (Phase 1) Database schema & ORM models
    │   ├── connection.py         # SQLAlchemy & PostGIS connection pool
    │   ├── init_db.py            # Automated database & PostGIS setup
    │   └── models.py             # Hotspot, Infrastructure, Baselines, Alerts
    │
    ...
    │   ├── osrm_client.py        # OSRM emergency routing engine (Fire & Ambulance)
    │   └── alert_service.py      # Twilio SMS / Email dispatcher (Fire & Medical)
    │
...
- **Spatial Database:** PostgreSQL 17 + PostGIS 3.5 Extension
...
Create a `.env` file in `AgniKavach/`:
```env
# Database Settings
DATABASE_URL=postgresql://postgres:arav123@localhost:5432/agnikavach_db

# NASA FIRMS API (Free key from firms.modaps.eosdis.nasa.gov)
